...
public class FirstPersonView {
	 ...
	 */
	private MazePanel graphicsPanel;
	...
	public void draw(MazePanel panel, int x, int y, int walkStep, int ang) {
        ...
        if (null == graphicsPanel.getGraphics()) {
        ...
        graphicsPanel.drawBackground(graphicsPanel.getBufferGraphics());
        // set color to white and draw what ever can be seen from the current position
        
        graphicsPanel.setColor(16777215); //RGB value for the color white
	...
	private void drawWall(Wall wall) {
		...
		graphicsPanel.setColor(wall.getRGBValue());
	...
	private boolean drawPolygons(int x1, int x2, int y11, int y12, int y21, int y22) {
		...
		while (x1i <= x2) {
			...
			graphicsPanel.fillPolygon(xps, yps, 4);

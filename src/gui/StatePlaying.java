...
public class StatePlaying extends DefaultState {
    ...
    public void start(Controller controller, MazePanel panel) {
    	...
    	winningState = (StateWinning) control.states[3];
        ...
        if(controller.getDriver() != null) {
        	//initialize robot and driver
        	control.getRobot().setMaze(controller);
        	control.getDriver().setDistance(controller.getMazeConfiguration().getMazedists());
        	control.getDriver().setDimensions(controller.getMazeConfiguration().getWidth(), controller.getMazeConfiguration().getHeight());
        	// create new thread
        	Thread thread = new Thread() {
        		public void run() {
        			try {
        				...
        				//switch to end screen if it fails
        				winningState.hasFailed = true;
        				control.switchFromPlayingToWinning(totalMoved); //return to the title screen
        			} 
        		}
        	};
        	//start the thread
    ...
    public boolean keyDown(UserInput key, int value) {
        ...
        case Up: // move forward
            ...
            if (isOutside(px,py)) {
            	//set energy to robot's energy if it exists
            	if(control.driver != null) {
            		winningState.setEnergyUsed((int)control.getDriver().getEnergyConsumption());
            		control.switchFromPlayingToWinning(totalMoved);
            	}
            	else {
            		winningState.setEnergyUsed(energyUsed);
            		control.switchFromPlayingToWinning(totalMoved);
            	}
            }
            break;
        ...
        //check if battery ran out
        if(control.getDriver() != null) {
        	if(control.getDriver().getEnergyConsumption() > 3000) {
        		winningState.setEnergyUsed((int)control.getDriver().getEnergyConsumption());
        		winningState.hasFailed = true;
        		control.switchFromPlayingToWinning(totalMoved);
        	}
